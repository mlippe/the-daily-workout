...
export function WorkoutRunner({
  ...
  const phaseTheme = useMemo(() => {
    switch (currentPhase) {
      case 'warmup':
        return {
          playBtn:
            'bg-emerald-400 text-neutral-950 hover:bg-emerald-300 shadow-[0_0_24px_rgba(52,211,153,0.45)]',
          playBtnPaused:
            'bg-emerald-500 text-neutral-950 hover:bg-emerald-400 shadow-[0_0_24px_rgba(52,211,153,0.6)] animate-pulse',
          badge:
            'bg-emerald-500/15 border border-emerald-500/40 text-emerald-400',
          repsCardHover:
            'hover:border-emerald-500/50 hover:shadow-[0_0_24px_rgba(52,211,153,0.15)]',
      ...
      case 'cooldown':
        return {
          playBtn:
            'bg-purple-400 text-neutral-950 hover:bg-purple-300 shadow-[0_0_24px_rgba(192,132,252,0.45)]',
          playBtnPaused:
            'bg-purple-500 text-neutral-950 hover:bg-purple-400 shadow-[0_0_24px_rgba(192,132,252,0.6)] animate-pulse',
          badge: 'bg-purple-500/15 border border-purple-500/40 text-purple-400',
          repsCardHover:
            'hover:border-purple-500/50 hover:shadow-[0_0_24px_rgba(192,132,252,0.15)]',
      ...
      default:
        return {
          playBtn:
            'bg-sky-400 text-neutral-950 hover:bg-sky-300 shadow-[0_0_24px_rgba(56,189,248,0.45)]',
          playBtnPaused:
            'bg-sky-500 text-neutral-950 hover:bg-sky-400 shadow-[0_0_24px_rgba(56,189,248,0.6)] animate-pulse',
          badge: 'bg-sky-500/15 border border-sky-500/40 text-sky-400',
          repsCardHover:
            'hover:border-sky-500/50 hover:shadow-[0_0_24px_rgba(56,189,248,0.15)]',
          ...
          controlHover:
            'hover:border-sky-500/50 hover:text-sky-300 hover:bg-sky-950/20',
  ...
  const quickSteps = useMemo(
    () => getQuickSteps(currentStep.exercise),
  ...
  const strokePercent =
    maxDuration > 0 ? (secondsRemaining / maxDuration) * 100 : 0;
  ...
  return (
    <div className='fixed inset-0 z-50 flex flex-col justify-between bg-black text-white select-none'>
      ...
      <header
        className={`border-b ${phaseTheme.headerBorder} bg-black/90 px-4 sm:px-6 pt-3 pb-2.5 backdrop-blur-md transition-colors duration-300`}
      >
        <div className='mx-auto max-w-2xl md:max-w-3xl'>
          <ProgressBar workout={workout} currentStep={currentStep} />
        </div>
      </header>
          ...
          <div className='flex w-full flex-col items-center'>
            ...
            <div className='w-full max-w-full sm:max-w-2xl md:max-w-3xl aspect-[3/2] max-h-[44vh] relative shrink-0 sm:rounded-3xl sm:mt-2 overflow-hidden bg-black'>
              ...
              >
                ...
                <div className='absolute top-3 inset-x-3 sm:top-4 sm:inset-x-4 z-20 flex items-center justify-between pointer-events-none'>
                  ...
                  <div
                    className={`pointer-events-auto rounded-full bg-black/60 px-3 py-1 font-mono text-xs font-semibold border ${phaseTheme.timerPill} backdrop-blur-md tabular-nums tracking-wider shadow-sm transition-colors`}
                  >
                  ...
                  <button
                    ...
                    aria-label={
                      isMuted ? 'Unmute countdowns' : 'Mute countdowns'
                    }
                  >
                    {isMuted ? (
            ...
            {currentStep.exercise.type === 'reps' ? (
              <div className='flex flex-col items-center justify-center shrink-0 my-4 sm:my-5'>
                ...
                <div className='mb-2'>
                  <span
                    className={`rounded-full ${phaseTheme.badge} px-3.5 py-1 font-mono text-xs uppercase tracking-widest font-semibold shadow-sm transition-colors`}
                  >
                    {currentStep.round
                      ? `Set ${currentStep.round} of 2`
                      : 'Standard Set'}
                  </span>
                </div>
                ...
                >
                  ...
                  <div
                    className={`mt-2 flex items-center gap-1.5 text-xs font-semibold ${phaseTheme.repActionText} transition-colors`}
                  >
              ...
              <div className='flex flex-col items-center justify-center shrink-0 my-4 sm:my-5'>
                <div className='relative flex h-32 w-32 sm:h-36 sm:w-36 md:h-40 md:w-40 items-center justify-center'>
                  ...
                  <div className='absolute flex flex-col items-center'>
                    <span className='font-mono text-4xl sm:text-5xl md:text-6xl font-black tabular-nums tracking-wide'>
                      {secondsRemaining}
                    </span>
      ...
      {/* Bottom Controls Bar */}
      <footer
        className={`border-t ${phaseTheme.footerBorder} bg-black px-4 py-4 transition-colors duration-300`}
      >
